	...
	<div class="good_detail" v-bind:class="{over:this.popupVisible1 || this.popupVisible2 || this.popupSale || this.popupParameter}">
		<div class="header">
			<a href="javascript:history.go(-1)">
				<img class="returnPic" src="../../assets/images/returnFront1.png">
			</a>
		...
		<!--固定底部栏-->
		<div class="shop-detail-bottom" @click="popUpShow">
			<div class="bottom-three">
				<div class="call-btn dlg-show">拨打电话，立即购买</div>
			</div>
		</div>
		<div class="dlg-mask" v-show="isShow" ></div>
		<div class="phone-dlg"  v-show="isShowBox">
			<h5>客服电话</h5>
			<p>4008716727</p>
			<p><span style="color: #f60; font-size: 1.2rem; text-align: center">拨打时间：周一至周五9:00-18:00!</span></p>
			<div class="btn-wrap">
				<span class="cancel" @click="popUpHide">取消</span>
				<span class="keep-call" @click="popUpHide"><a href="tel:4008716727">继续拨打</a></span>
			</div>
		</div>
	</div>
</template>
...
//				console.log(this.urlRemain)
			...
			wechatShare(){
				let _this = this;
				let Price,yuanPrice;
				if(_this.goodDetail.promotionPrice){
					Price=_this.goodDetail.promotionPrice;
					yuanPrice=_this.goodDetail.salesPrice;
				}else{
					Price=_this.goodDetail.salesPrice	
				};
				let link = window.location.href
				let wcs= new WeChatShareMixin(_this.goodDetail.name,
												_this.pics[0],
												 _this.goodDetail.salesPrice + '元',
												link,
												() => {
														// 成功的回调，如果没有可以不写
			...
			// 			desc: _this.goodDetail.salesPrice + '元',
			// 			original:yuanPrice+'yuan',
			// 			imgUrl: _this.pics[0],
			// 			link: link,
			// 			success: () => {
			// 				// 成功的回调，如果没有可以不写
