    ...
    <div class="allType">
        <div class="return_money_apply" v-if="moneyApply==1">
            ...
            <div class="return">
                ...
                <span class="returnNum">￥{{ entitys.refundMoney }}</span>
            </div>
        ...
        <div class="return_good_apply" v-if="moneyApply==2">
            ...
            <div class="container">
                ...
                <div class="z_photo">
                    ...
                    <div class="z_file" v-if="!isIos" v-show="showAddPic" id="fileBox" ref="camera_box" ></div>

                    <span class="z_addImg" v-for="(item,index) in imgUrls"  v-if="(!isWeChat && !isIos) || (isIos)  && imgUrls[0] !=''" @click="removeEchoWeChatPic(index)">
                        <img :src=item alt="" width="100" height="100">
                    </span>

                   
                    <div v-if="(!isWeChat && !isIos) && imgUrls[0] !=''" class="weChatDiv" v-for="(item,index) in imgUrls">
                        <span class="z_addImg">
                            <img :src=item alt="" @click="removeEchoWeChatPic(index)" class="echoWeChatPic">
                        </span>
                    </div>
                </div>
            </div>
...
//
                ...
                this.$resource (url ).get({ id: this.$route.query.refundCode }).then((response) => {
                    if( response.status == 200 ){
                        if( response.body.code == 200 ){
                            if( response.body.entity.refundType === 1 ){
                                this.moneyApply = 1
                                this.entitys = response.body.entity
                                this.enterReason = response.body.entity.applyAccount
                                this.selectReason = response.body.entity.refundReason
                            }else if( response.body.entity.refundType === 2 ){
                                this.moneyApply = 2
                                this.entitys = response.body.entity
                                this.enterReason = response.body.entity.applyAccount
                                this.selectReason = response.body.entity.refundReason
                                this.imgUrls = response.body.entity.refundGoodsImg.split(',')
            ...
            //删除回现的图片
            removeEchoWeChatPic(index){
                this.$nextTick(()=>{
                    var echoWeChatPics = document.getElementsByClassName('z_addImg')
                    if( echoWeChatPics.length > 0 ){
                        MessageBox.confirm('确定要删除该图片吗?').then(action => {
                            echoWeChatPics[index].remove()
                            if( echoWeChatPics.length >= 4 ){
                                this.showAddPic = false
                            }else {
                                this.showAddPic = true
                            }
...
//                            debug: true, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
            ...
            submitApplyRefundMoney(){
                ...
                let url = HOST_HDPORDERS + '/order/front/editRefund/'
                /* this.params = {
                    orderItemId: this.entitys.orderItemId,
                    refundMoney: this.entitys.refundMoney,
                    refundReason: this.selectReason,
                    refundType: this.entitys.refundType,
                    applyAccount: this.enterReason,
                    id:Number( this.$route.query.refundCode )
                }
