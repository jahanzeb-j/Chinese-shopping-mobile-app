<template>
  <div class="all">
    ...
    <div class="shopping_mon">
      ...
      <div class="shopping_money">项内容
        <span>{{checkboxModel.length}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    //       if( this.checkboxModel[i] == this.list[j].id){
  ...
  methods: {
    getList() {
      ...
      if (this.loginCode != "") {
        ...
        //         });
        //添加header
        this.$resource(url).get({ userId: this.userId, collectionType: 1, page: 1 }).then((response) => {
    ...
    delete1(index) {
      ...
      if (this.loginCode != "") {
        ...
        //         });
        //添加header
        this.$resource(url).get({ id: this.checkboxModel.join(',') }).then((response) => {
          if (response.status == 200) {
            this.getList();
          }
        })
