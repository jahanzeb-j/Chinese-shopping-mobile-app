<template>
  <div class="newCode" @click = "blurDelete()">
    ...
    <!--<div class="newCode-input">-->
      <div class="newCode-input">
        <input type="password"  @focus = "focusDelete1()" v-if="!isPsd1" placeholder="新密码（密码为6-20位字母数字组合）" id="admin"  class="newCode-input-admin" v-model="admin">
        <input type="text"  @focus = "focusDelete1()" v-if="isPsd1" placeholder="新密码（密码为6-20位字母数字组合）" id="admin"  class="newCode-input-admin" v-model="admin">
        <input type="password"  @focus = "focusDelete2();adminTest()" v-if="!isPsd2" placeholder="确认新密码" class="newCode-input-admin" v-model="verification">
        <input type="text"  @focus = "focusDelete2();adminTest()" v-if="isPsd2" placeholder="确认新密码" class="newCode-input-admin" v-model="verification">
...
//        window.localStorage.setItem('token',tokens);
      ...
      newCode: function () {
        //修改密码接口
        let url = HOST_HDPUSER+'/user/forgetPassword';
      ...
      back: function () {
        window.location.href = window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1)+"forgetPassword?onLine=1"
      }
    }
  }
</script>
