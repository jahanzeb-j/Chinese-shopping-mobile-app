<template>
    <div class="themeDetail" @click="hiddenInput($event)">
        ...
        <span v-if="lookmore == '没有更多了'" style="width: 100%;padding: 2.2rem;background: #f4f4f4;">
        ...
        <div class="bottomInpt" v-show="isShowarea" :class="{'release-box-up':releaseBoxUpStatus}">
            <textarea rows="3" cols="20" maxlength="400"  id="releaseThemeMsg" 
                v-model="releaseThemeMsg" 
                @focus="releaseBoxUp" 
                @blur="releaseBoxDown" 
                ref="Msginput"  
                v-on:keyup.13="enterMsg" 
...
//            获取昵称
            ...
            setId:function(){
                ...
                var thumbParams = window.Android.getLoginCode()
                let loginCode = thumbParams.split(',')[0]
                let userId = thumbParams.split(',')[1]
                this.user = userId
            },
            ...
            getCommentData(index){
                ...
                this.$resource (url ).get({ 'externalId': this.$route.query.id,'size':this.size}).then((response) => {
                    if( response.status == 200 ){
                        ...
                        console.log('response',this.msgMasterItems);
...
//                                 this.msgMasterItems[i].createTime=this.msgMasterItems[i].replace(/\-/g, "\/")
            ...
            //点击分享的时候
            shareNow(){
                var androidShare = window.Android.share()
            },
            ...
            //删除
            deleteMsg(index){
                var thumbParams = window.Android.logincode()
                let loginCode = thumbParams.split(',')[0]
                let userId = thumbParams.split(',')[1]
                if( loginCode != '' ){
                    this.isCancel=true
            ...
            deleteMsgChild(item){
                var deleteMsgChildParams = window.Android.logincode()
                let loginCode = deleteMsgChildParams.split(',')[0]
                let userId = deleteMsgChildParams.split(',')[1]
                if( loginCode != '' ){
                    this.isCancel=true
            ...
            enterGoodDetail(commodityId){
                ...
                var categoryId = this.$route.query.categoryId
                if( theme_list==1 ){
                    window.Android.intent( window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1) + 'good_detail?commodityId='+commodityId+'&id='+this.$route.query.id+'&theme_detail=1'+'&categoryId='+this.$route.query.categoryId+'&theme_list=1' )
//                    window.location.href = window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1) + 'good_detail?commodityId='+commodityId+'&id='+this.$route.query.id+'&theme_detail=1'+'&categoryId='+this.$route.query.categoryId+'&theme_list=1'
                    ...
                    window.Android.intent( window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1) + 'good_detail?commodityId='+commodityId+'&id='+this.$route.query.id+'&theme_detail=1' )
...
//                             return function () {
//                                 window.Android.intent( window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1) + 'good_detail?commodityId='+commodityId[i]+'&id='+id+'&theme_detail=1' )
...
//                 }
            ...
            tuwenEnterDetail2(commodityId){
                let id = this.$route.query.id
                window.Android.intent( window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1) + 'good_detail?commodityId='+commodityId+'&id='+id+'&theme_detail=1' )
            },
            //点赞
            thumbTheme(){
                var thumbParams = window.Android.logincode()
            ...
            //请求点赞接口判断点赞大拇指的颜色
            requestThumbTheme(){
                var requestThumbThemeAndroid = window.Android.getLoginCode().split(',')
                let url = HOST_HDPTHEME + '/themeStatistics/isFabulous'
                let id = this.$route.query.id
                this.params = {
                    themeId: id,
                    userId: requestThumbThemeAndroid[1]
                }
                this.$resource(url).get(this.params ).then((response) =>{
...
//           点击留言的时候
            releaseTheme2(index){
                var releaseThemeParams = window.Android.logincode()
                let loginCode = releaseThemeParams.split(',')[0]
                let userId = releaseThemeParams.split(',')[1]
                if( loginCode != '' ){
            ...
            //点击评论的时候
            releaseTheme(){
                var releaseThemeParams = ''
                if(window.Android){
                    releaseThemeParams = window.Android.logincode()
                }else{
                    releaseThemeParams = '2,2'
                }
            ...
            // 输入框向上滑动
            releaseBoxUp(){
                this.releaseBoxUpStatus = true
            },
            // 输入框向下滑动
            releaseBoxDown(){
                // this.releaseBoxUpStatus = false
            },
            //点击其他地方的时候是input消失
            hiddenInput(event){
...
//                  留言
                        ...
                        let body=[
                            {
                                child: [{}],
                                commentContent: this.releaseThemeMsg,
                                commentStar: 0,
                                createTime: "",
                                display: 0,
